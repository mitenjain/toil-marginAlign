...
def setup(job, input_file, n, down_checkpoints):
    ...
    job.fileStore.logToMaster(" Starting the merge sort got ID %s" % input_filestore_id)
...
def make_file_to_sort(file_name, lines, line_length):
    with open(file_name, 'w') as fileHandle:
        for i in xrange(lines):
            line = "".join(random.choice('actgACTGNXYZ') for i in xrange(line_length - 1)) + '\n'
            fileHandle.write(line)
...
def main():
    ...
    Job.Runner.startToil(Job.wrapJobFn(setup, os.path.abspath('file_to_sort.txt'), int(options.N), False,
                         memory='1000M', cores=1, disk="100M"), options)
